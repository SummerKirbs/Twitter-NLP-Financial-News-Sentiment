...
The model was tuned using RandomizedSearchCV with 10-Fold Cross-Validation, scored on macro-F1. Macro-F1 was chosen because it weights each class equally regardless of size, penalizing poor performance on any single sentiment class.

*   **Accuracy**: 0.739
*   **Macro F1**: 0.739
*   **AUC (per class)**: 0.84-0.86

The alignment between accuracy and macro-F1 (both at 0.739) is a meaningful signal as it indicates the model performs consistently across all sentiment classes, rather than inflating overall accuracy by over-predicting a dominant class.

F1 score itself balances precision (how often a predicted label is correct) and recall (how often the true label is captured), making it a more complete measure than accuracy alone in classification tasks. Optimizing for macro-F1 during cross-validation therefore encouraged the model to treat all sentiment classes as equally important during training.

The consistently high AUC values (0.84–0.86) across all classes further reinforce this, suggesting the model has learned meaningful patterns and can reliably distinguish between different sentiment classes across a wide range of decision thresholds.
...
*   **Author**: [SummerKirbs](https://github.com/SummerKirbs) [Truongnathan56](https://github.com/Truongnathan56)
